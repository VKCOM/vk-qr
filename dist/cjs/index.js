...
var qrcodegen = new function () {
  ...
  this.QrCode = function (version, errCorLvl, dataCodewords, mask) {
    ...
    this.toSvgString = function () {
      ...
      var logoColor = options.logoColor || LOGO_DEFAULT_COLOR;
      var suffix = options.suffix || 0;
      ...
      if (isShowLogo) {
        if (!logoData) {
          ...
          parts.push('<image preserveAspectRatio="xMidYMid slice" clip-path="url(#logo-mask-' + suffix + ')" style="width: 750px; height: 750px;" width="750" height="750" transform="translate(' + position + ',' + position + ') ' + scale + '" xlink:href="' + logoData + '" />');
        }
      }

      var backgroundSize = 99 * size;
      var qrBackground = '';
      var qrTransform = 'translate(0,0)';

      if (isShowBackground) {
        var qrScale = (qrSize - 20 * 2) / qrSize; // 0.756972112
        var padding = backgroundSize / qrSize * 21;
        var radius = Math.ceil(backgroundSize / (qrSize / 36)); // 7.11111111 = 256px/36px radius
