...
var qrcodegen = new function () {
  ...
  var BACKGROUND_DEFAULT_COLOR = '#FFF';
  var FOREGROUND_DEFAULT_COLOR = '#000';
  ...
  this.QrCode = function (version, errCorLvl, dataCodewords, mask) {
    ...
    this.getPixel = function (x, y) {
      var isShowLogo = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : true;
    ...
    this.toSvgString = function () {
      ...
      for (var y = 0; y < size; y++) {
        ...
        for (var x = 0; x < size; x++) {
          ...
          yCoord = y + topPadding;
          var neighbors = this.getNeighbors(x, y, isShowLogo);
          var path = '';
          var selector = '';
      ...
      var pointPosition = (size - qrcodegen.qrBorder) * qrcodegen.incTileSize;
      parts.push('<use fill-rule="evenodd" transform="translate(0,0)" xlink:href="#point"/>');
      parts.push('<use fill-rule="evenodd" transform="translate(' + pointPosition + ',0)" xlink:href="#point"/>');
      parts.push('<use fill-rule="evenodd" transform="translate(0,' + pointPosition + ')" xlink:href="#point"/>');
      if (isShowLogo) {
        if (!logoData) {
          parts.push('<use style="width: 750px; height: 750px;" fill="none" fill-rule="evenodd" transform="translate(' + position + ',' + position + ') ' + scale + '" xlink:href="#vk_logo"/>');
        } else {
          parts.push('<image style="width: 750px; height: 750px;" transform="translate(' + position + ',' + position + ') ' + scale + '" href="' + logoData + '" />');
        }
      }

      var pixelSize = 99 * size;
      var qrBackground = '';
