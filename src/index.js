...
var qrcodegen = new function() {
  ...
  const QR_SIZE_PIXELS = 256;
  const BACKGROUND_DEFAULT_COLOR = '#000';
  const FOREGROUND_DEFAULT_COLOR = '#F00';
  ...
  this.QrCode = function(version, errCorLvl, dataCodewords, mask) {
    ...
    this.toSvgString = function(qrSize = DEFAULT_SIZE, className = '', withLogo = true, withBackground = true, options = {}) {
      ...
      for (let y = 0; y < size; y++) {
        ...
        for (let x = 0; x < size; x++) {
          ...
          yCoord = y + topPadding;
          const neighbors = this.getNeighbors(x, y, withLogo);
          let path = '';
      ...
      if (withBackground) {
        let padding = (pixelSize / QR_SIZE_PIXELS) * 33;
   ...
   */
  this.createQR = (text, qrSize = DEFAULT_SIZE, className = '', withLogo = true, withBackground = true, options = {}) => {
    const segs = this.QrSegment.makeSegments(text);
    const svg = this.QrCode.encodeSegments(segs, this.QrCode.Ecc.QUARTILE, 1, 40, -1, true).toSvgString(qrSize, className, withLogo, withBackground, options);
    return svg;
  }
