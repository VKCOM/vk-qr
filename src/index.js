...
var qrcodegen = new function() {
  ...
  this.QrCode = function(version, errCorLvl, dataCodewords, mask) {
    ...
    this.getPixel = function(x, y, withLogo = true) {
    ...
    this.toSvgString = function(qrSize = DEFAULT_SIZE, className = '', withLogo = true) {
      ...
      return `<svg version="1.1" viewBox="0 0 ${99 * size} ${99 * size}" width="${qrSize}px" heigth="${qrSize}px"${className ? ` class="${className}"` : ''} xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
   ...
   */
  this.createQR = (text, qrSize = DEFAULT_SIZE, className = '', withLogo = true) => {
    const segs = this.QrSegment.makeSegments(text);
    const svg = this.QrCode.encodeSegments(segs, this.QrCode.Ecc.QUARTILE, 1, 40, -1, true).toSvgString(qrSize, className, withLogo);
    return svg;
  }
